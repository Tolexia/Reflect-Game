...
import { forwardRef, useRef, useMemo, useImperativeHandle, useLayoutEffect } from 'react'
...
function createEvent(api, hit, intersect, intersects) {
  ...
  const glowing = []
  for(const raycasted of intersects)
  {
    const uuid = raycasted.object.uuid
    if(!glowing.includes(uuid)) glowing.push(uuid)
  }
  const game_over = (glowing.length == api.objects.length)
...
export const Reflect = forwardRef(({ children, onUpdate, start: _start = [0, 0, 0], end: _end = [0, 0, 0], bounce = 10, far = 100, ...props }, fRef) => {
  bounce = (bounce || 1) + 1
  ...
  useLayoutEffect(() => {
    ...
    scene.current.traverse((object) => {
      if (object.isMesh && (object.onRayOver || object.onRayOut || object.onRayMove)) {
